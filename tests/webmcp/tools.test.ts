...
const actions: WebMcpActions = { getState: () => state, listSources: () => [], setLayerVisibility: (layerId, visible) => ({ layerId, visible }), setTimeWindow: (window) => ({ window }), setArea: (area) => area, inspectEvidence: () => null, analyzeCoverage: () => ({ ready: [] }), createLensDraft: (title) => ({ title, status: "draft" }), undoLastAgentChange: () => ({ undone: false }) };
...
test("exposes the accepted ten-tool semantic contract with strict schemas", () => {
  const tools = createEarthLensTools(actions);
