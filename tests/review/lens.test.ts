...
import { createSituationLensDraft, reviewSituationLensDraft, reviseSituationLensDraft } from "../../app/domain/review/lens.ts";
...
test("review records a real human-only acknowledgement without changing draft status", () => {
  const draft = createSituationLensDraft({ title: "Lens", area: { latitude: 1, longitude: 2, radiusKm: 5, label: "Area", updatedBy: "human" }, timeWindow: "24h", evidence: [evidence], coverage: [], createdAt: "2026-08-30T12:00:00Z", revision: 2 });
  const review = reviewSituationLensDraft(draft, "2026-08-30T12:10:00Z");
  assert.deepEqual(review, { reviewedAt: "2026-08-30T12:10:00Z", reviewedBy: "human", draftRevision: 2 });
