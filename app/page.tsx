...
const layerInfo: Record<LayerId, { label: string; source: string; provider: "usgs" | "eonet" | "open-meteo" | "nasa-firms"; color: string; limitation: string }> = {
  earthquakes: {
    label: "Earthquakes",
    source: "USGS",
    provider: "usgs",
...
export default function Home() {
  const [activeLayers, setActiveLayers] = useState<LayerId[]>(["earthquakes", "air-quality", "natural-events", "thermal-hotspots"]);
  ...
  const stateRef = useRef({ activeLayers, timeWindow, selection, earthquakes, areaEarthquakes, earthquakeState, naturalEvents, areaNaturalEvents, naturalEventState, airQuality: areaAirQuality, airQualityState, thermalHotspots, areaThermalHotspots, thermalHotspotState });

  useEffect(() => {
    stateRef.current = { activeLayers, timeWindow, selection, earthquakes, areaEarthquakes, earthquakeState, naturalEvents, areaNaturalEvents, naturalEventState, airQuality: areaAirQuality, airQualityState, thermalHotspots, areaThermalHotspots, thermalHotspotState };
  }, [activeLayers, timeWindow, selection, earthquakes, areaEarthquakes, earthquakeState, naturalEvents, areaNaturalEvents, naturalEventState, areaAirQuality, airQualityState, thermalHotspots, areaThermalHotspots, thermalHotspotState]);
  ...
  useEffect(() => {
    ...
    const allEvidence = () => [...stateRef.current.earthquakes, ...stateRef.current.naturalEvents, ...stateRef.current.thermalHotspots, ...(stateRef.current.airQuality ? [stateRef.current.airQuality] : [])];
    const scopedEvidence = () => [...stateRef.current.areaEarthquakes, ...stateRef.current.areaNaturalEvents, ...stateRef.current.areaThermalHotspots, ...(stateRef.current.airQuality ? [stateRef.current.airQuality] : [])];
    const sourceStates = () => ({ usgs: stateRef.current.earthquakeState, eonet: stateRef.current.naturalEventState, "open-meteo": stateRef.current.airQualityState, "nasa-firms": stateRef.current.thermalHotspotState });
    const tools = createEarthLensTools({
      ...
      setArea: (area) => { rememberAgentChange(); setSelectedObservation(null); setAirQuality(null); setAirQualityState({ status: "loading", requestedAt: new Date().toISOString() }); setThermalHotspots([]); setThermalHotspotState({ status: "loading", requestedAt: new Date().toISOString() }); setSelection(area); log(`Agent focused the map on “${area.label}”.`); return { area, revision: revisionRef.current, reversible: true }; },
      ...
      focusPlace: async (query, radiusKm) => {
        ...
        const [currentAir, currentThermalHotspots] = await Promise.all([
        ...
        return { ok: true as const, data: { area, match: resolution.candidate, airQuality: currentAir.status === "ready" ? currentAir.data : null, airQualityStatus: currentAir.status, thermalHotspots: currentThermalHotspots.status === "ready" ? currentThermalHotspots.data : [], thermalHotspotStatus: currentThermalHotspots.status, revision: revisionRef.current, reversible: true } };
      },
    });
  ...
  const executeAssistantAction = useCallback(async (action: AssistantAction): Promise<AssistantActionResult> => {
    ...
    const envelope = JSON.parse(result.content[0]?.text ?? "{}") as { ok?: boolean; data?: { area?: InvestigationArea; airQuality?: EvidenceRecord | null; airQualityStatus?: string; thermalHotspots?: EvidenceRecord[]; thermalHotspotStatus?: string; superseded?: boolean }; error?: { message?: string } };
    ...
    if (action.name === "focus_place" && envelope.data?.airQuality) {
      ...
      return {
        ok: true,
        summary: `Focused the map on ${envelope.data.area?.label ?? action.query ?? "the requested place"} and refreshed air and satellite evidence`,
        detail: `${envelope.data.area?.label ?? action.query}: US AQI ${evidence.attributes.usAqi} — ${category.label}. ${activity.headline}. ${activity.general}${evidence.attributes.dominantPollutant ? ` Likely AQI driver: ${evidence.attributes.dominantPollutant}.` : ""} PM2.5 ${evidence.attributes.pm2_5} µg/m³; PM10 ${evidence.attributes.pm10} µg/m³. This is a modelled CAMS estimate, observed ${new Date(evidence.observedAt).toLocaleString()}; check official local monitoring before making health or safety decisions. ${describeThermalRefresh(envelope.data.thermalHotspotStatus, envelope.data.thermalHotspots, stateRef.current.timeWindow)}`,
      };
    }
  ...
  const selected = [...earthquakes, ...naturalEvents, ...thermalHotspots, ...(areaAirQuality ? [areaAirQuality] : [])].find((item) => item.id === selectedObservation);
      ...
      <section className="workspace">
        <aside className="rail">
          ...
          <AssistantChat
            workspace={{
              ...
              sourceStates: { usgs: { status: earthquakeState.status }, eonet: { status: naturalEventState.status }, "open-meteo": { status: airQualityState.status }, "nasa-firms": { status: thermalHotspotState.status } },
              evidence: [...areaEarthquakes, ...areaNaturalEvents, ...areaThermalHotspots, ...(areaAirQuality ? [areaAirQuality] : [])].map((item) => ({
          ...
          <div className="sectionHead"><span>LIVE SIGNALS</span><span>{activeLayers.length}/4</span></div>
          <div className="signalList">
            {(Object.keys(layerInfo) as LayerId[]).map((id) => {
              const info = layerInfo[id];
              const sourceState = id === "earthquakes" ? earthquakeState : id === "natural-events" ? naturalEventState : id === "thermal-hotspots" ? thermalHotspotState : airQualityState;
              ...
              return (
                <button className={`signal ${on ? "active" : ""}`} key={id} onClick={() => toggleLayer(id)}>
                  ...
                  <span><small>{info.label}</small><strong>{id === "earthquakes" ? earthquakeSummary : id === "natural-events" ? naturalEventSummary : id === "thermal-hotspots" ? thermalHotspotSummary : airQualitySummary}</strong><em>{info.source} · {sourceStateLabel(sourceState)}</em></span>
                  <b>{on ? "✓" : "+"}</b>
                </button>
              );
            })}
        ...
        >
          <ArcgisInvestigationMap
            ...
            onAreaChange={(nextArea) => { agentUndoRef.current = []; revisionRef.current += 1; setSelectedObservation(null); setAirQuality(null); setAirQualityState({ status: "loading", requestedAt: new Date().toISOString() }); setThermalHotspots([]); setThermalHotspotState({ status: "loading", requestedAt: new Date().toISOString() }); setSelection(nextArea); log("You revised the investigation area.", "human"); }}
          />
          ...
          {panel === "uncertainty" && (
            <div className="evidenceCard">
              ...
              <div className="sourceStrip"><span>{selected ? selected.provider === "usgs" ? "USGS" : selected.provider === "eonet" ? "NASA EONET + origin" : selected.provider === "nasa-firms" ? "NASA FIRMS · VIIRS" : "Open-Meteo + CAMS" : "Public sources"}</span><span>{selected ? new Date(selected.observedAt).toLocaleString() : "live when available"}</span></div>
          ...
          {panel === "about" && <SidePanel title="A map you can question" eyebrow="ABOUT EARTH LENS" onClose={() => setPanel(null)}><p>Earth Lens is a shared spatial evidence workspace. Your agent operates semantic WebMCP tools—not map buttons—while every action remains visible and reversible.</p><p className="fineprint">Four signals use live public data from USGS, NASA EONET, NASA LANCE/FIRMS VIIRS, and Open-Meteo/CAMS. Thermal hotspots are satellite detections, not confirmed wildfires. Earth Lens is not an official emergency alert.</p></SidePanel>}
