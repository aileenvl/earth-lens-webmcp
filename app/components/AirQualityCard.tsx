...
export function AirQualityCard({ evidence, selected, onInspect }: { evidence: EvidenceRecord; selected: boolean; onInspect: () => void }) {
  ...
  return (
    <button className={`airQualityCard ${selected ? "selected" : ""}`} onClick={onInspect} aria-label={`Inspect current modelled air quality: AQI ${evidence.attributes.usAqi}, ${description.label}`}>
